...
export function getCityPageMetadata(city: CityInfo): Metadata {
  ...
  const title = `Déménagement ${city.nameCapitalized} : 3 devis minimum en 3 min | Moverz`;
  const description = `3 min · IA fiabilise le volume · 3 devis minimum comparables · Déménageurs contrôlés à ${city.nameCapitalized} · 0€ · Sans démarchage. ${cityHint(
    city.slug,
