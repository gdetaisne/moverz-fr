  ...
  const items = [
    {
      ...
      title: "Pros vérifiés",
      description:
        "Solidité financière vérifiée + Historique litiges analysé. Seuls les professionnels fiables reçoivent votre dossier.",
    },
    {
      icon: (
        <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      title: "Dossier prêt",
      description:
        "Un seul inventaire détaillé → tous les déménageurs chiffrent le même volume. Fini les devis incomparables.",
    },
    {
      icon: (
        <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      description:
        "Votre dossier reste anonyme jusqu'à ce que vous choisissiez. Vous gardez le contrôle total de vos échanges.",
    },
    {
      icon: (
        <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      description:
        "Comparez en toute liberté, décidez sans pression. 100% gratuit, sans frais cachés, sans harcèlement téléphonique.",
    },
  ];
    ...
    <div className="space-y-8">
      <div className="text-center space-y-3">
        ...
        <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white">
          4 garanties anti-arnaque
        </h2>
        <p className="text-white/80 max-w-3xl mx-auto text-sm md:text-base leading-relaxed">
          Pros vérifiés, dossier prêt pour devis, anonymat et comparaison claire : tout est fait pour
          éviter les arnaques et les mauvaises surprises.
        </p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:gap-6 max-w-5xl mx-auto">
        {items.map((item, idx) => (
          <div
            key={item.title}
