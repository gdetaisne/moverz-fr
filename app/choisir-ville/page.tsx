...
export default function ChoisirVillePage() {
  ...
  const quoteUrlForCity = (citySlug: string) =>
    buildTunnelUrl({ citySlug, from: "/choisir-ville/" });
    ...
    <main className="bg-hero min-h-screen">
      ...
      <section className="section section-light">
        <div className="container max-w-5xl">
            ...
            {Object.entries(citiesByRegion).map(([region, cities]) => (
              <div key={region} className="space-y-4">
                ...
                <div className="flex items-center gap-3 px-2">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-brand-turquoise/10 to-[#4FB8B8]/20">
                    <svg className="h-4 w-4 text-v4-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
                    </svg>
                  </div>
                  <h3 className="text-base font-bold text-v4-text">{region}</h3>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {cities.map((city) => (
                    <a
                      ...
                      className="group relative overflow-hidden flex items-center justify-between gap-4 rounded-xl border border-v4-border bg-white p-4 shadow-sm transition-all duration-300 hover:shadow-md hover:border-brand-turquoise/50 hover:-translate-y-0.5"
                    >
                      {/* Filament subtil */}
                      ...
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="font-bold text-v4-text group-hover:text-v4-accent transition-colors">
                            {city.nameCapitalized}
                          </div>
                          <div className="text-xs text-v4-text-secondary">{city.description}</div>
                        </div>
                      </div>
                      
                      {/* Arrow */}
                      <svg className="h-5 w-5 text-v4-accent group-hover:translate-x-1 transition-transform duration-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="mt-10 relative overflow-hidden rounded-2xl border border-v4-border bg-gradient-to-br from-white to-[#FAFBFC] p-8">
            <div className="absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-brand-turquoise/30 via-[#4f46e5]/20 to-[#22c55e]/30" />
            
            <div className="text-center mb-6">
              <h3 className="text-lg font-bold text-v4-text mb-2">Liste alphabétique</h3>
              <p className="text-sm text-v4-text-secondary">Toutes nos villes en un coup d'œil</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-2">
              {CITIES.sort((a, b) => a.nameCapitalized.localeCompare(b.nameCapitalized)).map((city) => (
                <a
                  key={city.slug}
                  href={quoteUrlForCity(city.slug)}
                  className="group flex items-center gap-2 text-sm text-[#4b5c6b] hover:text-v4-accent font-medium transition-colors py-1.5"
                >
          ...
          <div className="mt-8 text-center">
            <div className="inline-block rounded-2xl border border-v4-border bg-white p-8 shadow-sm hover:shadow-md transition-shadow duration-300">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-bold text-v4-text">Votre ville n'est pas listée ?</h3>
              </div>
