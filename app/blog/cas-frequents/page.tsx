...
export const metadata: Metadata = getFullMetadata(
  "blog/cas-frequents",
  "Cas fréquents déménagement : accès, objets lourds, checklists | Moverz",
  "Guides ultra pratiques sur les cas fréquents qui font dérailler un déménagement : accès, stationnement, portage, objets lourds, météo, organisation."
);

export default function BlogCasFrequentsPage() {
  const links = LONGTAIL_LINKS;
      ...
      <section className="relative overflow-hidden text-white">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 space-y-6">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold">
            Cas fréquents : guides ultra pratiques
          </h1>
          <p className="text-lg md:text-xl text-white/85 max-w-3xl leading-relaxed">
            Accès, stationnement, portage, objets lourds, météo, timing… ces sujets “niche” sont
            ceux qui créent le plus de surprises quand ils ne sont pas cadrés.
          </p>
        </div>
      </section>
        ...
        <div className="container max-w-5xl space-y-8">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-semibold text-[var(--color-text)]">
              Tous les cas fréquents ( {links.length} )
            </h2>
            <p className="text-sm md:text-base text-[var(--color-text-secondary)]">
              Des contenus courts à scanner, mais assez précis pour rendre vos devis comparables et
              éviter les galères le jour J.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-[var(--color-border)] bg-white p-6 md:p-8 text-center space-y-4">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] max-w-2xl mx-auto">
              Pour approfondir : checklists complètes, conseils et guides prix.
            </p>
              ...
              >
                <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--color-text-secondary)]">
                  {l.tags.slice(0, 3).map((t) => (
                    <span
                      key={t}
                      className="rounded-full bg-[#F0F4F8] px-2 py-0.5 font-semibold text-[11px] text-[#2B7A78]"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <p className="text-sm text-[var(--color-text-secondary)] line-clamp-2">{l.description}</p>
              </a>
            ))}
          </div>
        </div>
