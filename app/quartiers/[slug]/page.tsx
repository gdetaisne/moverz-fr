...
export default function QuartiersGenericPage({ params }: PageProps) {
  const city = getCityBySlug(params.slug);
  ...
  const quoteUrl = buildTunnelUrl({ citySlug: city.slug, from: `/quartiers-${city.slug}/` });
        ...
        <div className="container max-w-5xl py-16 md:py-24">
          <div className="text-center space-y-6">
            ...
            <div className="pt-4">
              <a
                href={quoteUrl}
                className="inline-flex items-center gap-2 rounded-full bg-white px-8 py-4 text-base md:text-lg font-semibold text-v4-text shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300"
              >
        ...
        <section className="section section-light">
          <div className="container max-w-5xl">
            <div className="text-center mb-12 space-y-4">
              ...
              <p className="text-v4-text-secondary max-w-2xl mx-auto">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {cityNeighborhoods.map((neighborhood) => (
                <div
                  key={neighborhood}
                  className="rounded-xl border border-v4-border bg-white p-4 hover:border-brand-turquoise/50 hover:shadow-md transition-all duration-200"
                >
                  <p className="font-medium text-v4-text">{neighborhood}</p>
                </div>
              ))}
            </div>

            <div className="mt-12 rounded-2xl border border-v4-border bg-gradient-to-br from-white to-[#FAFAFA] p-8">
              <h3 className="text-xl font-semibold text-v4-text mb-4">
                Pourquoi les {isParis ? "arrondissements" : "quartiers"} importent pour un déménagement
                ?
              </h3>
              <div className="space-y-3 text-v4-text-secondary">
                <p>
                  <strong className="text-v4-text">Accès et stationnement :</strong>{" "}
                  {isParis
                  ...
                  Les déménageurs doivent prévoir les autorisations et le matériel adapté
                  (monte-meubles, diables).
                </p>
                <p>
                  <strong className="text-v4-text">Type d'immeubles :</strong>{" "}
                  {isParis
                ...
                <p>
                  <strong className="text-v4-text">Formalités locales :</strong> Autorisation de
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {citySuburbs.map((suburb) => (
                <div
                  ...
                  className="rounded-xl border border-v4-border bg-white p-4 hover:border-brand-turquoise/50 hover:shadow-md transition-all duration-200"
                >
                  <p className="font-medium text-v4-text">{suburb}</p>
                </div>
              ))}
            </div>
          </div>
